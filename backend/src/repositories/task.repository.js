class TaskRepository {
  ...
  createTask = async (taskData, files = []) => {
    ...
    const rawTask = await this.Task.create({ ...taskData, position: count });

    const task = await this.Task.findById(rawTask._id)
      .populate('creatorId assignees', 'displayName email avatarUrl avatar');
