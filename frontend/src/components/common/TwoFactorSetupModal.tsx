...
export function TwoFactorSetupModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px] bg-white border-slate-200" aria-describedby="Trình thiết lập xác thực 2 lớp" onInteractOutside={(e) => {
        ...
        {step === 1 && (
          <div className="space-y-6 mt-2">
            <div className="flex flex-col sm:flex-row gap-6 items-center">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 shrink-0 w-44 h-44 flex items-center justify-center overflow-hidden">
                {isLoading || !qrCodeUrl ? (
              ...
              <div className="space-y-3 flex-1 w-full">
                <p className="text-sm font-medium text-slate-900 border-b pb-2">1. Quét mã QR</p>
                <p className="text-xs text-slate-600">Mở ứng dụng Authenticator và quét mã bên cạnh. Hoặc nhập mã bí mật theo cách thủ công:</p>
                <code className="block w-full bg-slate-100 p-2 rounded text-center font-mono text-sm tracking-widest font-bold text-slate-800 break-all select-all">
                  {secret}
                </code>
              </div>
            </div>
            ...
            <div className="space-y-3 border-t pt-4">
              <p className="text-sm font-medium text-slate-900">2. Nhập mã xác nhận</p>
              <p className="text-xs text-slate-600 mb-2">Sau khi quét luồng cấu hình sẽ tạo dãy 6 chữ số ngẫu nhiên thay đổi tự động. Hãy nhập mã hiện tại của bạn:</p>
              <div className="flex justify-center">
                <Input
                  ...
                  placeholder="000000"
                  className="text-center text-3xl font-bold tracking-[0.4em] h-14 w-64 focus-visible:ring-indigo-500"
                />
              </div>
            </div>

            <DialogFooter>
              <Button variant="outline" onClick={handleClose} disabled={isLoading} className="border-slate-300">
                Hủy
              </Button>
              <Button onClick={handleVerify} disabled={isLoading || code.length !== 6} className="bg-indigo-600 hover:bg-indigo-700 text-white w-full sm:w-auto">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 rounded-lg border border-slate-200">
              {backupCodes.map((bc, idx) => (
                <div key={idx} className="font-mono text-sm tracking-widest font-bold text-center py-2 bg-white rounded-md shadow-sm border border-slate-200 text-slate-800 select-all">
                  {bc}
                </div>
              ))}
            </div>
            ...
            <DialogFooter className="mt-6">
              <Button variant="outline" onClick={copyBackupCodes} className="bg-white border-slate-300 w-full sm:w-auto">
                {isCopied ? <CheckCircle2 className="w-4 h-4 mr-2 text-emerald-500" /> : <Copy className="w-4 h-4 mr-2" />}
                {isCopied ? "Đã chép mã" : "Sao chép mã"}
              </Button>
              <Button onClick={handleClose} className="bg-indigo-600 hover:bg-indigo-700 text-white w-full sm:w-auto mt-2 sm:mt-0">
